...
Grammar(
	...
	'pathRelativeSchemelessURL := 'pathRelativeURL ~ ":", //pathRelativeURL can't start with URLscheme
	'URLpathSegment := ('URLunit.rep) | 'singleDotPathSegment | 'doubleDotPathSegment, // URLunit can't be /,?, singleDotPathSegment, doubleDotPathSegment
	...
	'URLcodePoint := 'reserved | 'unreserved | 'unicode,
 	...
 	'unicode := "U+"( ("0" ~
 						(("0" ~ "A" ~ 'unicodeHEX ) | (("[1-9]".regex|"[A-F]".regex) ~ 'unicodeHEX.rep(2,2))))  //u+00A0 - u+0FFF
 					| (("[1-9]".regex | "A" | "B" | "C" | "E") ~ 'unicodeHEX.rep(3,3))		//u+1000-u+CFFF, u+E000-u+EFFF
 					| ("10" ~ ( (('digit | "[A-E]".regex) ~ 'unicodeHEX.rep(3,3)) 	//u+10000-u+10EFFF
 							| ("F" ~ ('digit | "[A-E]".regex) ~ 'unicodeHEX.rep(2,2)) 	//u+10F000-u+10FEFF
 							| ("FF" ~ ('digit | "[A-E]".regex) ~ 'unicodeHEX)		//u+10FF00-u+10FFEF
 							| ("FFF" ~ ('digit | "[A-D]".regex))			//u+10FFF0-u+10FFFD
 						) )		
 					| ("D" ~ "[0-7]".regex ~ 'unicodeHEX.rep(2,2)) 			//u+D000-u+D7FF
 					| ("F" ~ ('digit | "A"|"B" | "C" |"E" | "F") ~ 'unicodeHEX.rep(2,2)) 	//u+F000-u+FCFF, u+FE00-u+FFFF
 					| ("FD" ~ ('digit | "A"|"B" | "C") ~ 'unicodeHEX)	//u+FD00 - u+FDCF
 					| ("FDF" ~ 'unicodeHEX)			//u+FDF0-u+FDFF

 	), 

 	'host := ('userinfo ~ "@").? ~('domain | 'ipv4dress | ("[" ~ 'ipv6address ~ "]")), //TODO missing ipfuture, zoneid
 	'domain := ('unreserved | 'percentEncodedByte | 'subdelims).rep,
